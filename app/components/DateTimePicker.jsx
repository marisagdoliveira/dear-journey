...
const DateTimePicker = ({ isOpen, onClose, onSave, entryDate, email, saveEntry}) => {
  ...
  // Set the initial date to today, but ensure it's not before the current year
  const [selectedDate, setSelectedDate] = useState(() => {
    const today = new Date();
    return new Date(Math.max(today.getFullYear(), entryDate ? new Date(entryDate).getFullYear() : currentYear), today.getMonth(), today.getDate());
  });
  ...
  useEffect(() => {
    if (entryDate) {
      ...
      if (entryYear < currentYear) {
        setSelectedDate(new Date(currentYear, new Date(entryDate).getMonth(), new Date(entryDate).getDate()));
      } else {
  ...
  const handleDateChange = (e) => {
    ...
    setSelectedDate(prevDate => {
      ...
      if (name === 'year') newDate.setFullYear(value < currentYear ? currentYear : value); // Enforce minimum year
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative bg-gradient-to-br from-[#a49ef6bf] to-[#4e44a7e3] border border-white/60 p-6 rounded-3xl shadow-lg z-50 w-[320px] h-[280px] max-w-[90%]">
        <div className='flex w-200px h-[50px] justify-center rounded-xl border-[1px] border-white/60 bg-[#6c73f171]'>
          <p className='flex w-[300px] justify-start pl-[17px] darker-grotesque-main relative top-1' style={{ fontSize: 24 }}>
            Notify your future self
            <MailHeart className='absolute size-5 flex top-3 w-30 right-5 text-[#bbb0ff]'/>
          </p>
        </div>
        <div className="relative flex justify-center gap-2 mb-4 pl-10 pt-6">
          ...
          <select
            name="year"
            value={selectedDate.getFullYear() >= new Date().getFullYear() ? selectedDate.getFullYear() : new Date().getFullYear()}
